...
-- env{envs[0]}: main env that is activated by the user to run the MSMFE app.
...
for env in envs:
    ...
    # Install env{envs[0]} - requires opensim conda install before dependencies or osim will break
    try:
        subprocess.run(
            ["conda", "env", "create", "-f", f"setup_{envs[0]}/{env}.yml"],
            check=True,
        )
    ...
    if os.path.isfile(f"setup_{envs[0]}/{env}_requirements.txt"):
        print(
        ...
        try:
            subprocess.run(
                [
                    ...
                    "install",
                    "-r",
                    f"setup_{envs[0]}/{env}_requirements.txt",
                ],
                check=True,
            )
